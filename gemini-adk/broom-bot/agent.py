...
class BroomBotAgent:
    ...
    def __init__(self, model: str = "gemini-2.5-pro"):
        ...
        #   - appointment_agent
        #   - check_booking_agent
        # Note: lead_analysis_agent is a SequentialAgent workflow with 3 phases:
        #   - planning_agent → analyst_agent → engineer_agent
        ...
        self.lead_analysis_agent = self._create_lead_analysis_agent()  # Sequential workflow
    ...
    def _create_engineer_agent(self) -> LlmAgent:
        ...
        return LlmAgent(
            name="engineer_agent",
            output_key="query_results",  # Store final results here
            instruction=(
                "You are an Engineer Agent for Lead Analysis at Broom Bot Inc.\n\n"
                "Original Plan:\n"
                "{planning}\n\n"  # Can access Planning Agent's output

                "Technical Specification:\n"
                "{analysis_plan}\n\n"  # Can access Analyst Agent's output

                "Your Role: Generate SQL, execute query, and present results.\n\n"
                "Process:\n"
                "1. Review both the plan and technical specification above\n"
                "2. Use rag_sql_fewshot_tool to get similar SQL query examples\n"
                "3. Generate optimized SQL query based on the technical spec:\n"
                "   - Use the tables, columns, joins, and filters specified\n"
                "   - Ensure proper SQL syntax\n"
                "   - Add appropriate ORDER BY for readability\n"
                "4. Use execute_lead_query_tool to run the SQL query\n"
                "5. Present results to user in a clear format\n\n"
                "SQL Generation Tips:\n"
                "- Use DATE('now', '-N days') for relative dates\n"
            ...
            description="Engineer Agent generates SQL, executes query, and presents results",
            model=self.model
        )

    def _create_lead_analysis_agent(self) -> SequentialAgent:
        """Create Lead Analysis Sequential Workflow"""
        return SequentialAgent(
            name="lead_analysis_workflow",
            agents=[
                self._create_planning_agent(),    # Phase 1: Plan (output_key="analysis_plan")
                self._create_analyst_agent(),     # Phase 2: Analyze (reads {analysis_plan}, output_key="technical_spec")
                self._create_engineer_agent()     # Phase 3: Execute (reads {analysis_plan} + {technical_spec})
            ],
            instruction="Analyze customer leads through automated 3-phase pipeline: Planning → Analysis → SQL Execution"
        )
